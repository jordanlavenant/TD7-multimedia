...
void display()
{
    ...
    // ! Distinctions des matrices model, view et proj dans le shader.
    ...
    // ! Passage de la position de la lumière au shader
    glm::vec4 lightPos = glm::vec4(0.0, 0.0, 10.0, 0.0);
...
void initVAOs()
{
    ...
    std::ifstream ifs(concat(MY_SHADER_PATH, "/meshes/ImperialShuttle.off"));
    if (!ifs)
    {
...
void initShaders()
{
    ...
    // ! Distinctions de la position de la lumière dans le shader.
    lightPosID = glGetUniformLocation(progid, "LightSource_position");
}
